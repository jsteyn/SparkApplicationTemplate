...
public class Retrieving {
    ...
    public static ArrayList<String> getPeople(char type) {
        String sql;
        if (type=='h')
        sql = "SELECT P.FirstName, P.Email, P.PersonID FROM HelperPool h JOIN Person P ON P.PersonID=h.PersonID";
        else sql = "SELECT P.FirstName, P.Email, P.PersonID FROM InstructorPool i JOIN Person P ON P.PersonID=i.PersonID";
